...
//    function WebBaseUpload($bgcolor='') {
//        if ($bgcolor) {
...
//    }
    ...
    static function uploadImage($upload, $dir, $img_name, $isPathRoot = true) {
        if ((isset($upload['error']) && $upload['error'] == 0) || (!isset($upload['error']) && isset($upload['tmp_name']) && $upload['tmp_name'] != 'none')) {
            ...
            if (!file_exists($dir)) {
                if (!WebBaseCommon::makeDir($dir)) {
                    return array('suc' => false, 'msg' => '文件目录不可写', 'path' => '');
                }
            }

            if (!WebBaseImage::checkImgType($upload['type'])) {
                return array('suc' => false, 'msg' => '不允许的文件类型', 'path' => '');
            }

            if (self::moveFile($upload, $img_name)) {
                $path = str_replace(ROOT, '', $img_name);
                return array('suc' => true, 'msg' => '上传成功', 'path' => $path);
            } else {
                return array('suc' => false, 'msg' => '上传失败', 'path' => '');
            }
        } else {
            return array('suc' => false, 'msg' => '上传失败', 'path' => '');
        }
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor = '') {
        ...
        if ($thumb_width == 0 && $thumb_height == 0) {
            return str_replace(ROOT, '', str_replace('\\', '/', realpath($img)));
        }

        /* 检查原始文件是否存在及获得原始文件的信息 */
        $org_info = @getimagesize($img);
        if (!$org_info) {
            $this->error_msg = sprintf("无原始文件", $img);
            $this->error_no = 16;
        ...
        if (!$this->check_img_function($org_info[2])) {
            $this->error_msg = sprintf("不支持的类型", $this->type_maping[$org_info[2]]);
            $this->error_no = 1;
        ...
        /* 创建当月目录 */
        if (empty($path)) {
            $dir = ROOT . $this->images_dir . '/' . date('Ym') . '/';
        } else {
            $dir = $path;
        }
        ...
        if (!file_exists($dir)) {
            if (!webbase_common::make_dir($dir)) {
                /* 创建目录失败 */
                $this->error_msg = sprintf("创建目录失败", $dir);
                $this->error_no = 15;
                return false;
            }
        }
        ...
        if (file_exists($dir . $filename)) {
            return str_replace(ROOT, '', $dir) . $filename;
        } else {
            $this->error_msg = "文件不可写";
            $this->error_no = 14;
    ...
    function add_watermark($filename, $target_file = '', $watermark = '', $watermark_place = '', $watermark_alpha = 0.65) {
        ...
        if ((!file_exists($filename)) || (!is_file($filename))) {
            $this->error_msg = sprintf("文件不存在", $filename);
            $this->error_no = 2;
        ...
        if ($watermark_place == 0 || empty($watermark)) {
            return str_replace(ROOT, '', str_replace('\\', '/', realpath($filename)));
        }
        ...
        if (!$watermark_handle) {
            $this->error_msg = sprintf("文件不支持水印", $this->type_maping[$watermark_info[2]]);
            $this->error_no = 3;
        ...
        if (!$source_handle) {
            $this->error_msg = sprintf("文件不支持水印", $this->type_maping[$source_info[2]]);
            $this->error_no = 4;
        ...
        $tmp = imagecopymerge($source_handle, $watermark_handle, $x, $y, 0, 0, $watermark_info[0], $watermark_info[1], $watermark_alpha);
    ...
    function validate_image($path) {
        ...
        /* 文件是否存在 */
        if (!file_exists($path)) {
            $this->error_msg = sprintf("水印图片不合法", $path);
            $this->error_no = 8;
            return false;
        }
        ...
        if (!$this->check_img_function($image_info[2])) {
            $this->error_msg = sprintf("文件不存在", $this->type_maping[$image_info[2]]);
            $this->error_no = 10;
            return false;
        }
     ...
     * @return void
     */
    static function moveFile($upload, $target) {
        if (isset($upload['error']) && $upload['error'] > 0) {
            return false;
        }

        if (!WebBaseCommon::moveUploadFile($upload['tmp_name'], $target)) {
            return false;
        }
